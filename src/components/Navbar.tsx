...
function MobileMore({ view, onView }: { view: View; onView: (v: View) => void }) {
  ...
  return (
    <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-11 space-y-2 sm:hidden">
      {views.map(item => {
        ...
        return (
          <Button
            ...
            size="icon"
            className="h-11 w-11 rounded-xl bg-background/95 shadow-md backdrop-blur"
      ...
      <ThemeToggle className="bg-background/95 shadow-md backdrop-blur" />
    </div>
  )
}
